...
export function Journey({ experience }: { experience: Experience[] }) {
  ...
  return (
    <section id="journey" aria-labelledby="journey-title" className="bg-gallery-white py-[90px] sm:py-32">
      <div className="page">
        <SectionHeading id="journey-title" kicker={t("journey.kicker")} title={t("journey.title")} aside={t("journey.aside")} />
...
function Role({ job, state }: { job: Experience; state: "idle" | "focus" | "dim" }) {
  ...
  return (
    ...
    >
      <Reveal stagger className="grid gap-4 py-10 md:grid-cols-12 md:gap-8 lg:py-12">
        <div className="md:col-span-3">
          ...
          {current && <p className="mt-3 text-control font-semibold text-launch-orange">{t("journey.currentRole")}</p>}
        </div>
        <div className="md:col-span-9">
          ...
          {job.points.length > PREVIEW && (
            ...
            >
              {expanded ? t("journey.showLess") : t("journey.showAll", { count: job.points.length })}
